...
formEkskul.addEventListener('submit', function(event) {
    ...
    //Disini saya akan menambahkan validasi untuk memastikan semua jenis digit pada nisn benar-benar angka, 
    // dan maks 10 digit
    
    if (!/^\d{10}$/.test(nisn)) {

    errorMsg.textContent =
    'NISN tidak valid! Harus berjumlah 10 digit angka.';

    errorMsg.classList.replace(
      'error-hidden',
      'error-visible'
    );
